...
RSpec.describe Calculators::CautionCalculator do
  ...
  describe "#expiry_date" do
    ...
    context "when a simple caution" do
      ...
      it "returns the `spent_simple` variant without doing any date calculations" do
        ...
        expect(calculator.expiry_date).to eq(ResultsVariant::SPENT_SIMPLE)
      end
    end

    context "when a conditional caution" do
      let(:caution_type) { CautionType::ADULT_CONDITIONAL_CAUTION }
      let(:known_date) { Date.new(2019, 1, 31) }
      let(:conditional_end_date) { nil }

      context "and difference between conditional end date and caution date less than 3 months" do
        let(:conditional_end_date) { Date.new(2019, 3, 30) }

        it "returns conditional end date" do
          expect(calculator.expiry_date.to_s).to eq(conditional_end_date.to_s)
        end
      end

      context "and difference between conditional end date and caution date greater than 3 months" do
        let(:conditional_end_date) { Date.new(2019, 5, 30) }
        let(:result) { Date.new(2019, 4, 30) }

        it "returns caution date plus 3 months" do
          expect(calculator.expiry_date.to_s).to eq(result.to_s)
        end
      end

      context "and difference between conditional end date and caution date is 2 months x days" do
