...
RSpec.describe Calculators::Multiples::Proceedings do
  ...
  let(:disclosure_checks_scope) { instance_double(scope, completed: [disclosure_check_first, disclosure_check_second, disclosure_check_third]) }

  let(:disclosure_check_first) { instance_double(DisclosureCheck, kind:, conviction_date:, no_drag_through?: false) }
  let(:disclosure_check_second) { instance_double(DisclosureCheck, kind:, no_drag_through?: false) }
  let(:disclosure_check_third) { instance_double(DisclosureCheck, kind:, no_drag_through?: true) }

  let(:check_result_first) { instance_double(CheckResult, expiry_date: Date.new(2015, 10, 31)) }
  let(:check_result_second) { instance_double(CheckResult, expiry_date: Date.new(2018, 10, 31)) }
  let(:check_result_third) { instance_double(CheckResult, expiry_date: Date.new(2016, 10, 31)) }
  ...
  describe "#spent_date" do
    context "when there is only one sentence and is a date" do
      before do
        allow(CheckResult).to receive(:new).and_return(check_result_first)
      end
    ...
    context "when there is at least one `never_spent` date" do
      before do
        allow(CheckResult).to receive(:new).and_return(check_result_first, check_result_second, check_never_spent)
      end
    ...
    context "when all individual spent dates are dates" do
      before do
        allow(CheckResult).to receive(:new).and_return(check_result_first, check_result_second, check_result_third)
      end
    ...
    context "when there is at least one `indefinite` date" do
      before do
        allow(CheckResult).to receive(:new).and_return(check_result_first, check_result_second, check_indefinite)
      end
  ...
  describe "#spent_date_without_relevant_orders" do
    let(:disclosure_checks_scope) { instance_double(scope, completed: [disclosure_check_first, disclosure_check_second]) }

    context "when filters our relevant orders" do
      context "and only some are relevant orders" do
        let(:disclosure_check_first) { instance_double(DisclosureCheck, no_drag_through?: true) }
        let(:disclosure_check_second) { instance_double(DisclosureCheck, no_drag_through?: false) }

        it "calculates the spent_date of the non-relevant orders" do
          # rubocop:disable RSpec/SubjectStub, RSpec/StubbedMock
          expect(calculator).not_to receive(:expiry_date_for).with(disclosure_check_first)
          expect(calculator).to receive(:expiry_date_for).with(disclosure_check_second).and_return("date")
      ...
      context "when all are relevant orders" do
        let(:disclosure_check_first) { instance_double(DisclosureCheck, no_drag_through?: true) }
        let(:disclosure_check_second) { instance_double(DisclosureCheck, no_drag_through?: true) }

        it "returns a nil spent_date" do
          # rubocop:disable RSpec/SubjectStub
          expect(calculator).not_to receive(:expiry_date_for).with(disclosure_check_first).and_call_original
          expect(calculator).not_to receive(:expiry_date_for).with(disclosure_check_second).and_call_original
    ...
    context "when there is at least one `never_spent` date" do
      ...
      before do
        allow(CheckResult).to receive(:new).with(disclosure_check: disclosure_check_first).and_return(check_result_first)
        allow(CheckResult).to receive(:new).with(disclosure_check: disclosure_check_second).and_return(check_never_spent)
      end
